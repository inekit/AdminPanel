        ...
        <CCardBody>
          <CTable align="middle" class="mb-0 border" hover responsive>
            <CTableHead color="light">
              <CTableRow>
                <CTableHeaderCell
                  v-for="fn in fieldNames"
                  :key="fn + 'header'"
                  class="text-center"
                  >{{ fn }}</CTableHeaderCell
                >
                <CTableHeaderCell class="text-center"
                  >Действия</CTableHeaderCell
                >
              </CTableRow>
            </CTableHead>
            <CTableBody>
              <CTableRow
                v-for="(row, i) in transformData(rows)"
                :key="i + 'row'"
              >
                <CTableDataCell
                ...
                >
                  <CFormInput
                    v-if="updatingId === rows[i]?.id"
                    v-model="formData[fields[j]?.name]"
                  />
                ...
                <CTableDataCell>
                  <div class="d-grid gap-2 d-md-flex justify-content-md-center">
                    ...
                    <CButton
                      ...
                      :color="'light'"
                      size="md"
                      @click="updatingId = false"
                      >Отменить</CButton
                    >
                    <CButton
                      v-else
                      v-for="(info, name) in actions"
                      :key="name + 'action'"
                      :color="info?.color"
                      size="sm"
                      @click="chooseAction(name, info, i, j, column)"
                      >{{ name }}</CButton
                    >
                  </div>
                </CTableDataCell>
